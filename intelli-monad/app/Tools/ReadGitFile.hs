...
module Tools.ReadGitFile
  where

import Prelude (Eq, Show(show), ($), (<>), null, return)
...
instance FromJSON ReadGitFile where
  parseJSON = withObject "ReadGitFile" $ \rawObj -> do
    let (obj, foundWarnings) = warnUnknownKeys ["path"] rawObj            -- <-- whitelist
    ReadGitFile
      <$> obj .: "path"      -- public name → internal field
      <*> pure (if null foundWarnings then Nothing else Just foundWarnings)

instance ToJSON ReadGitFile where
  toJSON v@(ReadGitFile {}) = object
    [ "path"    .= (gitFilePath v)
    ]
...
instance HasFunctionObject ReadGitFile where
  ...
  getExamples = Just $ [ Example "Read the README.md in the root directory of the git repository." $ object
                         [ "path"    .= ("README.md" :: Text)
                         ]
                       ]
...
instance Tool ReadGitFile where
  ...
  toolExec args = liftIO $ do
    ...
    let requested = dir </> unpack args.gitFilePath
    ...
    if canonDir `isPrefixOf` canon
      then
          ...
          (\(e :: SomeException) -> return ReadGitFileOutput
                   { contents = "Error reading file: " <> pack (show e)
