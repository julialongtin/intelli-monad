...
import IntelliMonad.BaseTypes (Content(Content), Context, HasFunctionObject(getFunctionDescription), Message(ToolCall, ToolReturn), MonadTerminal(termInput, termOutput), Prompt, PromptEnv(inputCallback, outputCallback), ToolProxy(ToolProxy), contextToolbox, Tool(toolFunctionName), User(Tool))
...
checkPolicy :: forall m. (MonadIO m, MonadFail m, MonadTerminal m) => ToolRegistry -> Content -> Prompt m Content
